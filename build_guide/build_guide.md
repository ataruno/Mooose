...
※2:
Type-Cは少し基板が大きいとのこと。基板をねじ止め固定仕様とすると干渉するためType-Bをお選びください。耐震シートを引く場合は収まります。  
追記(25/3/4)：3/4以降、ケースを改良しType-C版でも干渉しないにしました。  
...
### 補足：ロータリーエンコーダの設定
右回り、左回り、押込みの3つを設定できます。  
青色の矢印をクリックすると右回り、左回り、押込みが切替わります。  
これはレイヤーごとに設定できます。  
![ ](./build_guide_image_remap/REの設定.jpg)
...
レイヤー1  
※トラボ感度変更のマクロキーは廃止しました。
代わりに感度違いのhexファイルを用意したため、お好きな感度をお使いください。  
デフォルトファイルが一番感度が高く、cpiの値が小さくなるほど感度が低くなります。  
[hexファイル置き場](https://github.com/ataruno/Mooose/tree/main/hex)  
